...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (startDate && endDate && dateRx.test(startDate) && dateRx.test(endDate)) {
      ...
      if (period === 'today') dateFilter = `DATE(cu.used_at) = CURRENT_DATE`
    ...
    const [summary, topCoupons, daily] = await Promise.all([
      prisma.$queryRawUnsafe<unknown[]>(`
        SELECT COUNT(*)::int AS total_uses, COALESCE(SUM(cu.discount), 0) AS total_discount, COUNT(DISTINCT cu.coupon_id)::int AS unique_coupons_used
        FROM coupon_usages cu JOIN coupons c ON cu.coupon_id = c.id
        WHERE c.tenant_id = ${user.tenant_id} AND ${dateFilter}
      `),
      prisma.$queryRawUnsafe<unknown[]>(`
        SELECT c.id, c.code, c.type, c.value, COUNT(cu.id)::int AS usage_count,
          COALESCE(SUM(cu.discount), 0) AS total_discount, COALESCE(AVG(s.final_total + cu.discount), 0) AS avg_order_value
        FROM coupons c
        LEFT JOIN coupon_usages cu ON c.id = cu.coupon_id AND ${dateFilter}
        LEFT JOIN sales s ON cu.sale_id = s.id
        WHERE c.is_deleted = false AND c.tenant_id = ${user.tenant_id}
        GROUP BY c.id ORDER BY usage_count DESC LIMIT 5
      `),
      prisma.$queryRawUnsafe<unknown[]>(`
        SELECT DATE(cu.used_at) AS date, COUNT(*)::int AS uses, COALESCE(SUM(cu.discount), 0) AS discount
        FROM coupon_usages cu JOIN coupons c ON cu.coupon_id = c.id
        WHERE c.tenant_id = ${user.tenant_id} AND ${dateFilter}
    ...
    return NextResponse.json({ success: true, data: { summary: Array.isArray(summary) ? summary[0] : {}, topCoupons, daily } })
  } catch (err) {
